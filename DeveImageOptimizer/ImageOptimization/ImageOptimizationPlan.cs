    ...
    public class ImageOptimizationPlan
    {
        ...
        public async Task<ImageOptimizationPlanResult> GoOptimize(string imagePath, ImageOptimizationLevel imageOptimizationLevel, List<string> tempFiles)
        {
            ...
            for (int i = 0; i < steps.Count; i++)
            {
                ...
                var resultString = $"{step.ToolExeFileName} Optimization successfull: {fileSizeBefore} => {fileSizeAfter} {warnIsBigger} (Elapsed: {Math.Round(result.ProcessResults.RunTime.TotalSeconds, 2)} seconds)";
